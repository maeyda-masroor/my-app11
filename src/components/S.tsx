...
const FlashSaleLayout: React.FC = () => {
  ...
  const products: Product[] = [
    { id: 1, name: "Product 1", price: "$25.00", image: "https://via.placeholder.com/150" , rating:4},
    { id: 2, name: "Product 2", price: "$30.00", image: "https://via.placeholder.com/150" ,rating:3 },
  ...
  return (
    <div className="flex flex-col h-screen bg-white p-12">
      {/* First Row */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          {products.slice(currentIndex, currentIndex + 4).map((product) => (
            <div key={product.id} className="p-4 bg-gray-100 rounded shadow w-[270px] h-[350px]">
              <div className="bg-red-600 w-10 h-5 m-2">40%</div>  
              <img
                src={product.image}
                alt={product.name}
                className="w-[200px] h-[200px] object-cover rounded p-4"
              />
              <h3 className="mt-2 text-lg font-semibold text-gray-700">{product.name}</h3>
              <p className="text-gray-500">{product.price}<del className="text-gray-500 ">150</del></p>
